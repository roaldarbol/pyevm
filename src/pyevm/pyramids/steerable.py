...
Filter design (analytic / one-sided, matching Wadhwa et al.)
------------------------------------------------------------
The pyramid uses an **analytic** (one-sided) representation, matching the
MATLAB reference code from Wadhwa et al. 2013.  Each oriented sub-band is
complex-valued with a meaningful local phase — essential for phase-based EVM.
...
**Oriented filters** (at each scale × orientation) — one-sided (analytic):
    H_k(r, θ) = bp(r) · g_k(θ)

where g_k is the **primary lobe only** (no antipodal):
    g_k(θ) = α · cos(θ − kπ/K)^{K−1}   for |θ − kπ/K| ≤ π/2, else 0

With only the primary lobe, the partition of unity over the half-plane gives:
    Σ_k g_k(θ)² = 1/2   (vs 1 for the two-sided case)

Reconstruction formula
----------------------
To recover the signal the oriented sub-band contribution is scaled by **2**:
    X_s = lp² · X_{s+1,up} + 2 · Σ_k sb_k · bp · g_k
        = lp² + 2 · (1−lp²)/2 = lp² + (1−lp²) = 1  ✓

The factor-of-2 compensates for the one-sided (analytic) representation and
matches `reconSCFpyrGen.m` from the MATLAB reference implementation.
"""
...
def _alpha_simoncelli(n: int) -> float:
    ...
    return math.sqrt(
        2 ** (2 * (n - 1)) * math.factorial(n - 1) ** 2 / (n * math.factorial(2 * (n - 1)))
    )


def _angular_filter(angle: torch.Tensor, orient_idx: int, n_orientations: int) -> torch.Tensor:
    """Simoncelli angular filter g_k = α · cos(θ − k·π/K)^{K−1} — primary lobe only.

    Only the **primary lobe** (|θ − target| ≤ π/2) is included, matching the
    analytic (one-sided) representation of ``buildSCFpyrGen.m``.  The sub-band
    is complex-valued, carrying a meaningful local phase that is used by
    phase-based EVM.

    The reconstruction formula scales the oriented contribution by **2** to
    compensate (see ``SteerablePyramid.collapse``).

    Returns a **real** tensor in [0, alpha]; the sub-band is complex because
    the one-sided filter breaks Hermitian symmetry in the DFT domain.
    """
    ...
    primary = torch.where(inside, cos_val.pow(n - 1), torch.zeros_like(diff))
